class AutoML {
    ...
    createDataset(params) {
        const request = {
            parent: this.autoMlBetaClient.locationPath(this.config.projectId, params.location),
            dataset: {
    ...
    getDataset(params) {
        const request = {
            name: this.autoMlClient.datasetPath(this.config.projectId, params.location, params.datasetId),
        };
    ...
    importDataSet(params) {
        const request = {
            name: this.autoMlClient.datasetPath(this.config.projectId, params.targetDataset.location, params.targetDataset.id),
            inputConfig: {
                gcsSource: {
        ...
        return new Promise((resolve, reject) => {
            this.autoMlClient.importData(request).then((responses) => {
    ...
    listModels(params) {
        const request = {
            parent: this.autoMlClient.locationPath(this.config.projectId, params.location)
        };
    ...
    deployModel(params) {
        const request = {
            name: this.autoMlClient.modelPath(this.config.projectId, params.location, params.modelId)
        }

        return new Promise((resolve, reject) => {
            this.autoMlClient.deployModel(request).then((responses) => {
                const operation = responses[0];
    ...
    undeployModel(params) {
        ...
        return new Promise((resolve, reject) => {
            this.autoMlClient.undeployModel(request).then((responses) => {
                const operation = responses[0];
    ...
    exportDataset(params) {
        ...
        return new Promise((resolve, reject) => {
            this.autoMlClient.exportData(request).then((responses) => {
    ...
    deleteDataset(params) {
        ...
        return new Promise((resolve, reject) => {
            this.autoMlClient.deleteDataset(request).then((responses) => {
                const operations = responses[0];
